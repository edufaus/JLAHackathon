        ...
        try {
            ...
            switch (error.code) {
                case 'auth/invalid-credential':
                    errorMessage = {code:"Invalid email or password"};
                    break;
                case 'auth/user-not-found':
                    errorMessage = {code:"No account found with this email"};
                    break;
                case 'auth/too-many-requests':
                    errorMessage = {code:"Too many attempts. Please try again later"};
                    break;
                default:
    ...
    <Card class="login-card w-full max-w-md mx-4 p-8 bg-black/40 backdrop-blur-xl border border-[#8B5CF6]/20 hover:border-[#8B5CF6]/40 transition-all duration-500">
        ...
        {/if}
            ...
            <div class="login-element flex justify-between text-sm">
                ...
                <a href="/auth/signup" class="text-[#8B5CF6] hover:text-[#cc99ff] transition-colors">
                    Create Account
                </a>
            </div>
        </div>
