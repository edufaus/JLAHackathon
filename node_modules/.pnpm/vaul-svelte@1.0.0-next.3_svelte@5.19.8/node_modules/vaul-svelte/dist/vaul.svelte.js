...
export class DrawerRootState {
    ...
    openTime = $state(null);
    dragStartTime = $state(null);
    dragEndTime = $state(null);
    lastTimeDragPrevented = $state(null);
    isAllowedToDrag = $state(false);
    nestedOpenChangeTimer = $state(null);
    pointerStart = $state(0);
    keyboardIsOpen = $state(false);
    previousDiffFromInitial = $state(0);
    drawerNode = $state(null);
        ...
        $effect(() => {
            ...
            return untrack(() => {
                const onVisualViewportChange = () => {
                    if (!this.drawerNode || !this.repositionInputs.current)
                        return;
    ...
    onPress = (e) => {
        if (!this.dismissible.current && !this.snapPoints.current)
            return;
        if (this.drawerNode && !this.drawerNode.contains(e.target))
            return;
    ...
    shouldDrag = (el, isDraggingInDirection) => {
        ...
        if (this.lastTimeDragPrevented !== null &&
    ...
    onDrag = (e) => {
        ...
        const draggedDistance = (this.pointerStart - (isVertical(this.direction.current) ? e.pageY : e.pageX)) *
            directionMultiplier;
        ...
        const drawerDimension = this.direction.current === "bottom" || this.direction.current === "top"
            ? this.drawerHeight
            : this.drawerWidth;
        ...
        if (this.snapPointsState.shouldFade ||
    ...
    resetDrawer = () => {
        ...
        if (this.shouldScaleBackground.current &&
            currentSwipeAmount &&
    ...
    cancelDrag = () => {
        if (!this.isDragging || !this.drawerNode)
            return;
    ...
    onRelease = (e) => {
        ...
        if (!this.shouldDrag(e.target, false) || !swipeAmount || Number.isNaN(swipeAmount))
            return;
        if (this.dragStartTime === null)
            return;
        const timeTaken = this.dragEndTime.getTime() - this.dragStartTime.getTime();
        const distMoved = this.pointerStart - (isVertical(this.direction.current) ? e.pageY : e.pageX);
        ...
        if (this.direction.current === "bottom" || this.direction.current === "right"
            ? distMoved > 0
            : distMoved < 0) {
            this.resetDrawer();
            this.onReleaseProp.current?.(e, true);
    ...
    onDialogOpenChange = (o) => {
        if (!this.dismissible.current && !this.open.current)
            return;
        if (o) {
...
class DrawerOverlayState {
    ...
    #hasSnapPoints = $derived.by(() => this.#root.snapPoints.current && this.#root.snapPoints.current.length > 0);
    #onmouseup = (e) => {
    ...
    props = $derived.by(() => ({
        ...
        "data-vaul-snap-points-overlay": this.#root.open.current && this.#root.snapPointsState.shouldFade
            ? "true"
            : "false",
...
class DrawerContentState {
    ...
    pointerStart = $state(null);
    lastKnownPointerEvent = $state(null);
    ...
    snapPointsOffset = $derived.by(() => $state.snapshot(this.#root.snapPointsState.snapPointsOffset));
    props = $derived.by(() => ({
        id: this.#id.current,
...
class DrawerHandleState {
    ...
    handleCycleSnapPoints = () => {
        ...
        const isLastSnapPoint = this.#root.activeSnapPoint.current ===
            this.#root.snapPoints.current?.[this.#root.snapPoints.current.length - 1];
        if (isLastSnapPoint && this.#root.dismissible.current) {
            this.#root.closeDrawer();
            return;
        }
        const currentSnapIndex = this.#root.snapPoints.current?.findIndex((point) => point === this.#root.activeSnapPoint.current);
...
// CONTEXT
////////////////////////////////////
export const [setDrawerRootContext, getDrawerRootContext] = createContext("Drawer.Root");
export function useDrawerRoot(props) {
...
export function useDrawerOverlay(props) {
    const root = getDrawerRootContext();
    return new DrawerOverlayState(props, root);
}
export function useDrawerHandle(props) {
    const root = getDrawerRootContext();
